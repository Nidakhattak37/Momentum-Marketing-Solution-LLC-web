...
const Navbar: React.FC<NavbarProps> = ({ currentView, setView }) => {
  ...
  return (
    <nav className={`sticky top-0 z-[100] border-b border-white/5 px-6 lg:px-10 py-4 lg:py-5 transition-all duration-700 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'} ${isMobileMenuOpen ? 'bg-black' : 'bg-black/90 backdrop-blur-2xl'}`}>
      <div className="max-w-[1800px] mx-auto flex items-center justify-between relative">
        ...
        <div className="hidden lg:flex items-center">
              ...
              >
                ...
                {item.sub && activeDropdown === item.id && (
                  <div className={`absolute top-full pt-4 ${item.id === 'srv' ? 'right-0 w-[520px]' : 'left-0 w-64'}`}>
                    <div className={`bg-[#070707] border border-white/10 rounded-xl shadow-[0_40px_80px_-20px_rgba(0,0,0,0.9)] overflow-hidden p-2 backdrop-blur-3xl submenu-container-reveal ${item.id === 'srv' ? 'grid grid-cols-2 gap-1' : 'space-y-1'}`}>
                      {item.sub.map((sub, sIdx) => (
                        <button
                          key={sIdx}
                          onClick={() => handleLinkClick(sub.id)}
                          className="w-full text-left px-4 py-4 bg-white/[0.02] hover:bg-white/[0.07] rounded-lg flex items-center justify-between group/sub transition-all duration-300 submenu-item-reveal border border-white/[0.03] hover:border-white/10"
                        ...
                        >
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-[9px] font-bold text-zinc-400 group-hover/sub:text-white uppercase tracking-widest transition-all duration-300 group-hover/sub:translate-x-1">
                              {sub.label}
                            </span>
                          </div>
                          ...
                          <div className="relative flex items-center justify-center w-4 h-4">
                            <div className="w-1 h-1 rounded-full bg-zinc-800 border border-white/10 group-hover/sub:bg-cyan-400 group-hover/sub:border-cyan-300 group-hover/sub:shadow-[0_0_10px_#00E5FF,0_0_20px_rgba(0,229,255,0.4)] transition-all duration-500" />
                            <div className="absolute inset-0 bg-cyan-400/20 rounded-full blur-md opacity-0 group-hover/sub:opacity-100 transition-opacity duration-700" />
                          </div>
                        </button>
                      ))}
                    </div>
          ...
          <div className="h-6 w-[1px] bg-white/10 mx-6 animate-in fade-in duration-1000 delay-500" />
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-right-4 duration-1000 delay-700">
      ...
      <style>{`
        ...
        @keyframes submenuContainerEntrance {
          0% { opacity: 0; transform: scale(0.98) translateY(-8px); filter: blur(4px); }
          100% { opacity: 1; transform: scale(1) translateY(0); filter: blur(0); }
        }
        .submenu-container-reveal {
          animation: submenuContainerEntrance 0.4s cubic-bezier(0.23, 1, 0.32, 1) forwards;
        }
        ...
        .submenu-item-reveal {
          opacity: 0;
          animation: subItemReveal 0.5s cubic-bezier(0.23, 1, 0.32, 1) forwards;
          animation-delay: calc(var(--sIdx) * 0.04s);
        }
      `}</style>
    </nav>
  );
